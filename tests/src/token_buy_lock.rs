...
fn test_buy_exact()
{
	...
	script_args.extend(buyer_lock_script_hash.to_vec());
	script_args.extend(sudt_type_script_hash.to_vec());
	script_args.extend(100u128.to_le_bytes().to_vec());
	let script_args: Bytes = script_args.into();
	let token_buy_lock_script = context.build_script(&out_point_token_buy_lock, script_args).expect("script");

	// Prepare Input Cells
	let mut inputs = vec![];
	let input_out_point = context.create_cell(CellOutput::new_builder().capacity(100_000_000_000_u64.pack()).lock(token_buy_lock_script.clone()).build(), Bytes::default());
...
fn test_buy_invalid_args()
{
	...
	script_args.extend(buyer_lock_script_hash.to_vec());
	script_args.extend(sudt_type_script_hash.to_vec());
	script_args.extend(100u64.to_le_bytes().to_vec());
	let script_args: Bytes = script_args.into();
	let token_buy_lock_script = context.build_script(&out_point_token_buy_lock, script_args).expect("script");
